...
const SeasonalDialModule: React.FC<TimeModuleProps> = ({ targetDate }) => {
  ...
  useEffect(() => {
    ...
    // --- SEASONS CONFIG ---
    const seasons = [
        { name: 'Winter', startAngle: 0, endAngle: Math.PI / 2, labelAngle: Math.PI / 4 },
        { name: 'Spring', startAngle: Math.PI / 2, endAngle: Math.PI, labelAngle: (3 * Math.PI) / 4 },
        { name: 'Summer', startAngle: Math.PI, endAngle: (3 * Math.PI) / 2, labelAngle: (5 * Math.PI) / 4 },
        { name: 'Fall', startAngle: (3 * Math.PI) / 2, endAngle: 2 * Math.PI, labelAngle: (7 * Math.PI) / 4 },
    ];
    ...
    labelsGroup.selectAll('.cardinal-group')
        .data(cardinalLabels)
        .enter()
        .append('g')
        .each(function(d) {
            ...
            g.append('text')
                ...
                .attr('class', 'font-mono text-[8px] sm:text-[10px] md:text-xs text-stone-500 fill-current font-bold uppercase tracking-wider')
                .text(d.name);
            ...
            g.append('text')
                ...
                .attr('text-anchor', d.align)
                .attr('class', 'font-mono text-[8px] sm:text-[9px] md:text-[10px] text-stone-600 fill-current uppercase tracking-wider')
                .text(d.date);
        });
    ...
    if (currentAngle < Math.PI / 2) activeSeasonIndex = 0; // Winter
    else if (currentAngle < Math.PI) activeSeasonIndex = 1; // Spring
    else if (currentAngle < 3 * Math.PI / 2) activeSeasonIndex = 2; // Summer
    ...
    const seasonProgress = Math.max(0, Math.min(1, angleInSeason / seasonDuration));
    ...
    curGroup.append('text')
        ...
        .attr('y', curLabelY)
        .attr('text-anchor', 'middle')
        .attr('class', 'font-mono text-[9px] sm:text-[10px] md:text-xs font-bold text-stone-100 fill-current tracking-widest')
        .text('YOU ARE HERE');
    ...
    curGroup.append('text')
        ...
        .attr('text-anchor', 'middle')
        .attr('class', 'font-mono text-[8px] sm:text-[9px] md:text-[10px] text-stone-400 fill-current uppercase tracking-wider')
        .text(now.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }).toUpperCase());
    ...
    tarGroup.append('text')
        ...
        .attr('class', 'font-mono text-[9px] sm:text-[10px] md:text-xs font-bold text-stone-300 fill-current tracking-widest')
        .text('THE WEDDING');
    ...
    tarGroup.append('text')
        ...
        .attr('text-anchor', 'end')
        .attr('class', 'font-mono text-[8px] sm:text-[9px] md:text-[10px] text-stone-400 fill-current uppercase tracking-wider')
        .text(targetDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }).toUpperCase());
